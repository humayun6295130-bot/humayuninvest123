...
export function AdminCommandBar({ onJump }: AdminCommandBarProps) {
    ...
    const counts = useMemo(() => {
        ...
        const depositsEtc = txs.filter((t: { type?: string }) => t.type !== "withdrawal").length;
        const balanceWithdrawals = txs.filter((t: { type?: string }) => t.type === "withdrawal").length;
        const investments =
            pendingInv?.filter(
                (row: { status?: string }) =>
                    row.status === "pending_payment_confirmation" || row.status === "payment_received"
            ).length ?? 0;
    ...
    }, [pendingTxs, pendingPay, pendingInv, kycQueue, supportQueue]);
        ...
        <Card className="border-amber-500/30 bg-gradient-to-r from-amber-500/5 via-background to-background">
            <CardContent className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                            <p className="font-semibold text-sm sm:text-base">Quick queue</p>
                            <p className="text-xs text-muted-foreground truncate">Tap to open the right section.</p>
                        </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 w-full lg:max-w-md">
                        ...
                        <Button type="button" onClick={goUserSearch} className="shrink-0 h-10 w-full sm:w-auto">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <Button type="button" size="sm" variant="outline" onClick={() => onJump("platform")}>
                        <FolderCog className="h-3.5 w-3.5 mr-1.5" />
                        Platform
                    </Button>
                    <Button type="button" size="sm" variant="outline" onClick={() => onJump("messages")}>
                        <Megaphone className="h-3.5 w-3.5 mr-1.5" />
                        Messages
                    </Button>
                    <Button type="button" size="sm" variant="outline" onClick={() => onJump("analytics")}>
